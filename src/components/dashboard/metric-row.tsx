...
function Tile({ label, target, prefix, suffix, decimals, change, up, index }:
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.04, duration: 0.28 }}
      style={{ padding: "16px 18px" }}>
      <p style={{ fontSize: 11, color: "var(--text-3)", marginBottom: 8, fontWeight: 500 }}>{label}</p>
      ...
      <p style={{ fontSize: 11, color: up ? "var(--green)" : "var(--red)", fontWeight: 500 }}>{change}</p>
    </motion.div>
  );
}
...
export function MetricRow() {
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 14, overflow: "hidden" }}>
      {METRICS.map((m, i) => (
        <div key={m.label} style={{ borderRight: i < METRICS.length - 1 ? "1px solid var(--border)" : "none" }}>
          <Tile {...m} index={i} />
        </div>
      ))}
    </div>
